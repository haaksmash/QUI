...
\subsubsection{MongoDB}
	...
	but the user can specify the port via passed in arguments. The \texttt{\_get\_interface()} method takes the model instance and sets each field as a key:value pair in the JSON
	dict.
...
\subsubsection{Pickling}
	Also supported is local storage of model instances, using the Python pickle module. Model instances are stored in the directory specified by the \texttt{\_path} variable, in
	the file $Modelname\backslash InstanceName$. Sadly, Python's pickle module will only pickle global classes (namely, not our Models), so this backend simply pickles a dict
...
(\texttt{NewBackEndMixins.py}) defining the necessary classes, and QUI will take care of performing the operations given a new Model.
...
without a working application online. In light of these, providing support for just the AppEngine database seemed both overly time-consuming and unlikely to benefit any customers, as they
...
QUI was developed and implemented as a team. The initial design was done entirely on whiteboards with both participants working together. Later in the process, pair programming was 
used, with both members working on one computer. This was the abstract base classes and later the classes for the mixins to inherit. The last stage of the project was actually 
developing for the backends. The backends were split between the two team members, though both had input in each backend --- for example, the decision to not support AppEngine
as a backend was made as a team.
